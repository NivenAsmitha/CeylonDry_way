...
function settingsLinkClass(active: boolean): string {
  return `inline-flex min-h-12 shrink-0 items-center border-b-2 px-1 text-sm font-bold transition focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-emerald-700 ${
    active
...
export function ProfilePage() {
  ...
  const workspace = user.roles.includes("DEVELOPER")
    ? { label: "Developer workspace", to: "/developer/users" }
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-6 p-6 sm:p-8 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-4 sm:gap-5">
            <div
              className="grid size-16 shrink-0 place-items-center rounded-full bg-emerald-700 text-xl font-black text-white ring-4 ring-emerald-50 sm:size-20 sm:text-2xl"
              aria-hidden="true"
            >
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link
              className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold text-slate-700 transition hover:border-emerald-300 hover:text-emerald-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700"
              to={workspace.to}
            >
              {workspace.label}
            </Link>
      ...
      <div className="mt-6 min-w-0">
            ...
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
              <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                ...
                <dl className="mt-7 divide-y divide-slate-200 border-y border-slate-200">
                  ...
                  <div className="grid gap-1 py-4 sm:grid-cols-[11rem_1fr] sm:gap-4">
                    <dt className="text-sm font-semibold text-slate-500">Email address</dt>
                    <dd className="break-all font-semibold text-slate-900">{user.email}</dd>
                  </div>
                  <div className="grid gap-1 py-4 sm:grid-cols-[11rem_1fr] sm:gap-4">
                    <dt className="text-sm font-semibold text-slate-500">Phone number</dt>
                    <dd className="font-semibold text-slate-900">{user.phone || "Not provided"}</dd>
                  </div>
                  <div className="grid gap-1 py-4 sm:grid-cols-[11rem_1fr] sm:gap-4">
                    <dt className="text-sm font-semibold text-slate-500">Member since</dt>
                    <dd className="font-semibold text-slate-900">{formatAccountDate(user.createdAt)}</dd>
                  </div>
                </dl>
              </section>

              <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 className="text-lg font-black text-slate-950">Account access</h2>
                <p className="mt-2 text-sm leading-6 text-slate-500">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {user.roles.map((role) => (
                    <span
                      className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-800 ring-1 ring-inset ring-emerald-200"
                      key={role}
                    >
                      {getRoleLabel(role)}
                    </span>
          ...
          {section === "edit" ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
              <h2 className="text-2xl font-black tracking-tight text-slate-950">Edit profile</h2>
              ...
              {profileSucceeded ? (
                <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-900" role="status">
                  Your profile was updated successfully.
                </div>
              ) : null}
              <form className="mt-7 max-w-2xl space-y-5" noValidate onSubmit={(event) => void profileForm.handleSubmit(submitProfile)(event)}>
                <FormField
                  id="profile-name"
                  label="Full name"
                  autoComplete="name"
                ...
                />
                <div>
                  <label className="mb-2 block text-sm font-semibold text-slate-800" htmlFor="profile-language">Language</label>
                  <select className="min-h-12 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 shadow-sm outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100" id="profile-language" {...profileForm.register("language")}>
                    {SUPPORTED_LANGUAGES.map((language) => (
                      <option value={language} key={language}>English</option>
                    ))}
                  </select>
                </div>
                <div className="flex flex-wrap gap-3 pt-2">
                  <button className="min-h-12 rounded-xl bg-emerald-700 px-6 font-extrabold text-white hover:bg-emerald-800 disabled:opacity-60" type="submit" disabled={profileForm.formState.isSubmitting}>
                    {profileForm.formState.isSubmitting ? "Saving changes…" : "Save changes"}
                  </button>
                  <Link className="inline-flex min-h-12 items-center rounded-xl border border-slate-300 px-6 font-bold text-slate-700" to="/profile?section=overview">Cancel</Link>
                </div>
              </form>
            </div>
          ) : null}
          ...
          {section === "security" ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
              <h2 className="text-2xl font-black tracking-tight text-slate-950">Change password</h2>
              ...
              {passwordError ? (
                <div className="mt-6">
                  <ErrorMessage message={passwordError} title="Password change failed" />
                </div>
              ) : null}
              <form className="mt-7 max-w-2xl space-y-5" noValidate onSubmit={(event) => void passwordForm.handleSubmit(submitPassword)(event)}>
                <FormField
                  id="current-password"
                ...
                />
                <div className="flex flex-wrap gap-3 pt-2">
                  <button className="min-h-12 rounded-xl bg-slate-950 px-6 font-extrabold text-white hover:bg-slate-800 disabled:opacity-60" type="submit" disabled={passwordForm.formState.isSubmitting}>
